...
export async function fetchOfficialBCVRate() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const val = Number(data?.promedio);
      if (val > 0) return { rate: val, source: 'DolarApi (BCV Oficial)' };
    }
  } catch (err) {
  ...
  if (!rate) {
    throw new Error('No se encontró la tasa USD en ninguna de las fuentes');
  }
